...
import {
  ...
  Trade,
  Withdrawal,
  CreateWithdrawalApiRequestSchema, WithdrawalApiResponseSchema
} from './types.js';
import {ethers, TypedDataDomain, TypedDataField} from "ethers";
import {SignTypedDataParameters} from "@wagmi/core";
import {Address, encodeFunctionData, EncodeFunctionDataParameters, formatUnits, Hex, TypedData} from "viem";
import {base64urlEncode, fromFundamentalUnits, generateOrderNonce, getDomain} from "./utils.js";
import ERC20Abi from "./ERC20Abi.js";
import ExchangeAbi from "./ExchangeAbi.js";
import { Decimal } from "decimal.js";
import {BitcoinWalletImpl} from "./bitcoin-wallet.js";
import { v4 as uuidv4 } from 'uuid'
import {quoteAmountMinusFeeToReceiveForSellingBase, quoteAmountRequiredForBuyingBaseIncludingFee} from "./quotes.js";
import {ExponentialBackoff, Websocket, WebsocketBuilder} from "websocket-ts";
import {
  AmmState, Balances, balancesTopic, BalancesUpdated,
  IncomingWSMessage, MarketAmmState, marketAmmStateTopic, myOrdersTopic, myTradesTopic, OHLCDuration, OrderBook,
  orderBookTopic, Prices, pricesTopic,
  Publishable,
  PublishableSchema,
  SubscriptionTopic, UpdatedBalance
...
export class FunkybitClient {
  ...
  async login(): Promise<boolean> {
    ...
    if (config.authorizedAddresses.length === 0) {
      ...
      if (authorizedWalletAuthToken != null) {
        const timestamp = new Date().toISOString()
        const authorizingWalletSignature = await this.params.bitcoinWallet.signMessage(
          this.params.bitcoinWallet.address,
    ...
    if (this.config !== undefined) {
      ...
      if (chain !== undefined) {
        ...
        if (symbol.chainId === 'bitcoin') {
          ...
          const address = (symbol.contractAddress?.split(':')?.length ?? 0) === 2 ? this.params.bitcoinWallet.ordinalsAddress : this.params.bitcoinWallet.address
          const signature = await this.params.bitcoinWallet.signMessage(
            address,
          ...
          return (await this.api.createWithdrawal({symbol: symbol.name, amount, nonce: timestamp.getTime(), signature})).withdrawal
        } else {
          const exchangeContractAddress = this.baseChain()!.contracts!.find(c => c.name === 'Exchange')!.address
          ...
          if (signature === null) {
            ...
            return (await this.api.createWithdrawal({
              symbol: symbol.name,
              amount,
      ...
      if (chain !== undefined) {
        if (symbol.chainId === 'bitcoin') {
          if (symbol.contractAddress === null) {
            const depositAddress = chain.contracts.find(c => c.name === 'CoinProxy')?.address
            if (depositAddress !== undefined) {
              const txHash = await this.params.bitcoinWallet.sendTransaction(depositAddress, amount)
              const response = await this.api.createDeposit({symbol: symbol.name, amount: amount, txHash})
          ...
          const depositAddress = chain.contracts.find(c => c.name === 'Exchange')?.address
          if (depositAddress !== undefined) {
            if (symbol.contractAddress === null) {
              const txHash = await this.params.evmWallet.sendTransaction(depositAddress, amount)
              const response = await this.api.createDeposit({symbol: symbol.name, amount: amount, txHash})
              return response.deposit
            } else {
              ...
              const response = await this.api.createDeposit({symbol: symbol.name, amount: amount, txHash: depositTx})
              return response.deposit
            }
          } else {
  ...
  async search(sort: CoinsSortOption, search: string | undefined, safeSearch: boolean | undefined): Promise<Coin[]> {
    if (!this.loggedIn) {
      throw new Error("Must be logged in first")
    }
    return (await this.api.listCoins({queries: {sort, search: search ?? "", 'safe-search': safeSearch ?? false}})).coins
  }
  ...
  private async connectWebsocket(refreshAuth: boolean = false) {
    ...
    this.ws?.close()
    const connectionUrl = apiBaseUrl.replace('http:', 'ws:').replace('https:', 'wss:') + '/connect'

    this.ws = new WebsocketBuilder(connectionUrl + authQuery)
      .withBackoff(new ExponentialBackoff(1000, 4))
      .onMessage((ws, m) => {this.handleMessage(ws, m)})
      .onOpen(() => {
    ...
    if (wallet instanceof BitcoinWalletImpl) {
      ...
      const signature = percentage
        ? await this.params.evmWallet.signTypedData(
          ...
          {
            sender:
              this.params.evmWallet.address.toString(),
            baseChainId: baseSymbol.chainId === 'bitcoin' ? 0 : BigInt(baseSymbol.chainId),
            baseToken:
              baseSymbol.contractAddress ??
              '0x0000000000000000000000000000000000000000',
            quoteChainId: baseSymbol.chainId === 'bitcoin' ? 0 : BigInt(quoteSymbol.chainId),
            quoteToken:
        ...
        : await this.params.evmWallet.signTypedData(
          ...
          },
          {
            ...
            baseToken:
              baseSymbol.contractAddress ??
              '0x0000000000000000000000000000000000000000',
            quoteChainId: quoteSymbol.chainId === 'bitcoin' ? 0 : BigInt(quoteSymbol.chainId),
            quoteToken:
              quoteSymbol.contractAddress ??
              '0x0000000000000000000000000000000000000000',
            amount: amount,
  ...
  async getQuote(market: MarketWithSymbolInfos, side: OrderSide, amount: bigint, inAsset: QuoteSymbol): Promise<Quote> {
    const quoteSymbol = inAsset === 'BTC' ? this.bitcoinSymbol() : this.usdcSymbol()
      ...
      try {
        ...
        interval = setInterval(() => {
          if (ammState !== undefined) {
            const adapterMarketState = this.adapterMarket().baseSymbol.name === quoteSymbol?.name ? {
              market: this.adapterMarket(),
              orderBook: this.adapterMarketOrderBook!,
              feeRates: this.config!.feeRates!
            } : null
            const quoteAmount = (side === 'Buy' ? quoteAmountRequiredForBuyingBaseIncludingFee(
                amount,
                market,
                ammState,
                adapterMarketState
  ...
  async placeOrder(quote: Quote, slippageTolerance: Decimal = new Decimal("0.02")) {
    ...
    const signatureAndSigningAddress = (await this.signOrderCreation(
      ...
      quote.inAsset === 'BTC' ? this.adapterMarket().baseSymbol : quote.market.quoteSymbol,
      quote.side,
      quote.amount,
      null,
      null
  ...
  subscribeToBalances(full: (balances: Balance[]) => void, update: (update: UpdatedBalance[]) => void): UnsubscriptionHandler {
    const handler = (message: Publishable) => {
      switch (message.type) {
        case 'Balances':
  ...
  subscribeToOrders(onOrders: (orders: Order[]) => void): UnsubscriptionHandler {
    const handler = (message: Publishable) => {
      switch (message.type) {
        case 'MyOrders':
  ...
  subscribeToTrades(onTrades: (trades: Trade[]) => void): UnsubscriptionHandler {
    const handler = (message: Publishable) => {
      switch (message.type) {
        case 'MyTrades':
  ...
  subscribeToPrices(marketId: string, duration: OHLCDuration, onPrices: (prices: Prices) => void): UnsubscriptionHandler {
    const handler = (message: Publishable) => {
      switch (message.type) {
        case 'Prices':
          onPrices(message)
